---
title: Call for posters at SDLCAI 2026
subtitle: Put your work on the wall at Marsio
summary: We invite applied and academic poster proposals about AI across the software development lifecycle for SDLCAI on 13 October 2026.
date: 2026-08-18
author: Juho Vepsäläinen
eyebrow: Call for posters
---

We are opening a call for posters for SDLCAI 2026. We plan to display between five and ten posters during the seminar at Marsio, Aalto University, on 13 October.

**[Submit your poster proposal by 27 September 2026.](/posters/#submit)** We review proposals as they arrive and make acceptance decisions on a rolling basis, so the call may close early if all available spaces have been allocated.

## What we are looking for

We welcome applied and academic work about AI across the software development lifecycle. A proposal might present research findings, an industry case study, an experiment, a method, a tool, a failure, or work in progress.

The subject can touch any part of software delivery: planning, requirements, architecture, implementation, review, testing, deployment, operations, maintenance, governance, developer experience, or organizational change. We are interested in concrete work that gives attendees something useful to question, compare, or take back to their own teams.

Product and commercial experience can be relevant, but the poster must teach or investigate something. Sales pitches and advertisements will not be accepted.

## Format and practical details

Posters must be A0 or A1 in portrait orientation. Accepted presenters are responsible for printing and bringing their own poster, installing it at the venue, and removing it after the event.

The poster session runs from 12:30 to 13:30 on 13 October at Marsio. A designated presenter should attend the session to discuss the work with seminar participants.

Each accepted poster includes one SDLCAI event ticket for its designated presenter.

## How selection works

We select proposals for their relevance to SDLCAI, specificity, learning value, and contribution to a balanced mix of academic and industry perspectives. Acceptance is rolling: proposals are reviewed as they arrive, and a poster space may be offered before the submission deadline.

The final deadline is 27 September 2026. Because only 5–10 spaces are available, applying early is encouraged.

**[Read the full brief and submit your proposal](/posters/#submit)**
