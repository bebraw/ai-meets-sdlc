...
const sourceDir = path.join(root, "assets/email/source");
const outputDir = path.join(root, "assets/email/exports");
const assets = [
  {
    name: "sdlcai-2026-header",
    label: "EVENT HEADER",
    width: 1200,
    height: 600,
  },
  {
    name: "sdlcai-2026-header-compact",
...
try {
  ...
  for (const asset of assets) {
    ...
    // Keep email assets within a conservative 500 KB budget.
    if (bytes >= 500_000)
      throw new Error(`${asset.name} exceeds the 500 KB export budget`);
    console.log(
  ...
  await page.setViewportSize({ width: 696, height: 680 });
  await page.goto("about:blank");
  await page.setContent(`<!doctype html><html lang="en"><meta charset="utf-8"><title>SDLCAI email graphics</title><style>
    * { box-sizing: border-box; } body { margin: 0; padding: 48px; background: #232323; color: #f6f4ef; font: 22px sans-serif; }
    h1 { font-size: 28px; margin: 0 0 36px; } main { display: grid; grid-template-columns: 1fr; gap: 40px; align-items: start; }
    figure { margin: 0; } figure:first-child { grid-column: 1 / -1; width: 600px; }
    ...
    </style><h1>SDLCAI 2026 / EMAIL HEADERS</h1><main>${cards.join("")}</main></html>`);
  await page.evaluate(() =>
